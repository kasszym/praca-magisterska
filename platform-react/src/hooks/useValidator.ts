export const useValidator = () => {
  const peselValidator = (value: string): string | undefined => {
    const pesel = value.toString();
    const age = checkAge(pesel);
    if (age < 18) {
      return 'Musisz mieć 18 lat.';
    }
    const result =
    ...
    if (!result) {
      return 'PESEL musi być prawdziwy.';
    }
    return undefined;
  };
  ...
  const postCodeValidator = (value: string): string | undefined => {
    const reg = /^[0-9]{2}-[0-9]{3}$/;
    const postCode = value.toString();
    if (!reg.test(postCode)) {
      return 'Kod pocztowy musi być prawdziwy.';
    }
    return undefined;
  };
